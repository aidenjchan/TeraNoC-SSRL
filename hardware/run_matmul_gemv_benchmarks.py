...
"""Run matmul/gemv terapool benchmarks sequentially.

Each job runs:
  app=<app> make benchmark config=terapool

from the hardware/ directory. Binaries are expected under
software/bin/apps/baremetal/terapool/ (built via auto_build.py).
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run matmul/gemv terapool benchmarks back to back.",
    )
    parser.add_argument(
        "--apps",
        nargs="+",
        default=DEFAULT_APPS,
        help="App names to benchmark (default: matmul/gemv f16/f32)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    log(f"===== Running {total} benchmarks (config={CONFIG}) =====")
